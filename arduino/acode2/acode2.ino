...
volatile char sdatain[2]; //1X2 ARRAY FOR INCOMING SERIAL DATA
...
{
  detachInterrupt(digitalPinToInterrupt(interruptPin));
  if(aaa==0){digitalWrite(12, HIGH);aaa++;}
  else if(aaa==1){digitalWrite(12, LOW);aaa=0;}
  spd=cnt/timesample*7.5;
  cnt=0;
...
void senddata(){
    Serial.print(cur,DEC); // MASSURE AND SEND CURRENT
  Serial.print("_");
...
// SEND TO PC:
    ...
    Serial.print(cur,DEC); // MASSURE AND SEND CURRENT
    //_________________________________________________________________________________________________________________________________________***NEED CHANGE
    Serial.print(spd,DEC); // NUMBER OF COUNTED INTERUPTS OF SPEED SENSOR. THESE NEEDS TO BE REPLACED WITH ACTUAL SPEED: 
