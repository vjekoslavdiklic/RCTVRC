  ...
  ISR(TIMER0_COMPA_vect) // TD TRIGGERED // Timer 0 - Broji impulse
{
  ...
  //cur=(float)analogRead(A7); //READ ANALOG VALUE ON CURRENT SENSOR
...
// READ NEW DATA:   
  while (Serial.available()) 
  {
    ...
    Serial.print(currmes(),6); // MASSURE AND SEND CURRENT cur
    Serial.write('\r');// PRINT /r ON SERIAL   
...
float currmes(){
  float output=0;
  output=(float)analogRead(currentpin);
  output=(output/(float)1023)*currange-currange/2;
  return output;
  }
