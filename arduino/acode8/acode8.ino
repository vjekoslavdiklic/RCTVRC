...
#define encoder_holes 10 //NUMBER OF HOLES IN ENCODER WHEEL
...
ISR(TIMER1_OVF_vect){ //TIMER1 OVERFLOW 
 ...
 /* ISR(TIMER1_COMPA_vect) // Tc TRIGGERED
{
  S1=TCNT0;
  TIMSK1=B00000001;
  OCR0A=S1+1;
...
*/
  ...
  ISR(TIMER0_COMPA_vect) // TD TRIGGERED
{
  ...
  spd=6*2000000/(Td);
  //if(((spd-spd_last)>350)&&(spd_last>2000)){
    //spd=spd_last;}
  //spd_last=spd;
  //8cur=(float)analogRead(currentpin); //READ ANALOG VALUE ON CURRENT SENSOR
  //cur=(cur/(float)1023)*currange-currange/2; //CALCULATE REAL CURRENT IN AMPERS
  cur=(((float)analogRead(currentpin))/(float)1023)*currange-currange/2; //CALCULATE REAL CURRENT IN AMPERS
  //cur=cur/2.5;
