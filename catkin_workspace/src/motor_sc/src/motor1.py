...
port0 = serial.Serial("/dev/ttyUSB1", baudrate=1000000)


out=minf_msg()


pub = rospy.Publisher('motor1_feedback',minf_msg, queue_size=10)
...
def listener():
    rospy.init_node('motor1_node', anonymous=True)
    rospy.Subscriber('motor1_cnt',mcnt_msg, callback)
