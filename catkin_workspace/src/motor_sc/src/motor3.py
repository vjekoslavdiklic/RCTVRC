...
port0 = serial.Serial("/dev/ttyUSB3", baudrate=1000000)


out=minf_msg()


pub = rospy.Publisher('motor3_feedback',minf_msg, queue_size=10)
...
def listener():
    rospy.init_node('motor3_node', anonymous=True)
    rospy.Subscriber('motor3_cnt',mcnt_msg, callback)
